...
// ----------------- GET PICKUP PERSONNEL (SORTED BY ADDRESS + PENDING) -----------------
export const fetchPickupPersonnel = async (address) => {
  try {
    const response = await API.get(`/pickup-personnel?address=${address}`);
    return response.data;
  } catch (error) {
    console.error("Error fetching pickup personnel:", error);
    return [];
  }
};
...
// ----------------- STRIPE PAYMENT -----------------
export const createCheckoutSession = async ({ userId, amount, type, requestId = null }) => {
  try {
