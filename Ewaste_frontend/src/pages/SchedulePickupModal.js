...
import { fetchPickupPersonnel } from "../api/api";
import API from "../api/api";

const SchedulePickupModal = ({ requestId, requestAddress, onClose, onScheduled }) => {
  ...
  const fetchPersonnel = async () => {
    try {
      const personnels = await fetchPickupPersonnel(requestAddress);

      console.log("Pickup Personnel Response:", personnels);
...
const handlePersonnelChange = (e) => {
  ...
  console.log("Selected pickup person:", selectedPerson);
...
const handleSchedule = async () => {
  ...
  if (!pickupPersonnelName || !pickupPersonnelEmail) {
    alert("⚠️ Pickup personnel information missing.");
    return;
  }
  ...
  try {
    ...
    const payload = {
      ...
      pickupPersonnelId: String(pickupPersonnelId),
      pickupPersonnelName: pickupPersonnelName,
      pickupPersonnelEmail: pickupPersonnelEmail
    };

    console.log("Scheduling Request Payload:", payload);

    const response = await API.put(
      `/ewaste/${requestId}/schedule`,
      payload
    );
              ...
              {pickupPersons.length > 0 ? (
                pickupPersons.map((p) => (
                  <option key={p.id} value={p.id}>
                    [{p.id}] {p.name} - {p.address} (Pending: {p.pendingCount})
                  </option>
                ))
              ) : (
