...
public class EwasteRequestController {
    ...
    public ResponseEntity<?> pickupResponse(@PathVariable Integer id,
        ...
        try {
            ...
            // 🚨 VERY IMPORTANT: Do NOT allow changes if already completed
            if (RequestStatus.Completed.equals(request.getStatus())) {
                return ResponseEntity.badRequest()
                        .body("Request already completed. Status cannot be modified.");
            }
            ...
            if ("ACCEPT".equalsIgnoreCase(response)) {
                ...
                if (PickupResponseStatus.ACCEPTED.equals(request.getPickupResponseStatus())) {
                    return ResponseEntity.badRequest()
                            .body("Pickup already accepted");
                }

                request.setPickupResponseStatus(PickupResponseStatus.ACCEPTED);
                request.setPickupRespondedAt(LocalDateTime.now());

                // Only set Scheduled if not completed
                ...
                if (PickupResponseStatus.REJECTED.equals(request.getPickupResponseStatus())) {
                    return ResponseEntity.badRequest()
                            .body("Pickup already rejected");
                }
                ...
                return ResponseEntity.badRequest()
                        .body("response must be ACCEPT or REJECT");
            }
    ...
    public ResponseEntity<?> completeRequest(@PathVariable Integer id,
        ...
        try {
            ...
            if (inputOtp == null || request.getCompletionOtp() == null
                    || !inputOtp.equals(request.getCompletionOtp())) {
            ...
            // ✅ Transaction (stripeSessionId intentionally NULL for offline flow)
            ...
            tx.setStripeSessionId(null); // Explicit
            ...
            emailService.sendTemplateEmail(
                    ...
                    Map.of(
                            ...
                            "pickupPerson", request.getPickupPersonnelName(),
                            "pickupDateTime",
                            request.getPickupDateTime() != null
                                    ? request.getPickupDateTime().toString()
                                    : "Not Scheduled",
                            "paidAmount", amount
                    )
