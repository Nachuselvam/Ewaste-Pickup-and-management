...
const API = axios.create({
  ...
  headers: { "Content-Type": "application/json" },
  auth: {
    username: "admin@gmail.com", // admin credentials
    password: "admin123",
  },
});
...
export const approveRequest = async (requestId) => {
  try {
    const res = await API.put(`/ewaste/${requestId}/approve`);
    return res.data;
  } catch (err) {
    console.error(`Approve request ${requestId} error:`, err.response || err.message);
    return { message: err.response?.data?.message || "❌ Failed to approve request." };
  }
};
...
export const rejectRequest = async (requestId, reason) => {
  try {
    ...
    return { message: err.response?.data?.message || "❌ Failed to reject request." };
  }
};
...
export const markRequestCompleted = async (requestId, otp) => {
  try {
    const res = await API.put(`/ewaste/${requestId}/complete`, { otp });
    return res.data;
  } catch (err) {
    console.error(`Error marking request ${requestId} as completed:`, err);
    return { message: err.response?.data?.message || "❌ Failed to complete request." };
  }
};
